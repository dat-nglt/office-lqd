            ...
            <Box className="p-0 space-y-3 pb-6">
                ...
                <Box className="pb-3 border-b border-gray-200 pt-1">
                    <Box className="flex-1 min-w-0 mb-2">
                        <Box className="flex gap-1 mb-1">
                            <Text className="font-bold text-sm text-gray-900 uppercase">
                                {work.workType === "project" ? "Dự Án" : "Dịch Vụ"}
                            </Text>
                            <Text className="font-bold text-sm text-gray-900 uppercase">-</Text>
                            <Text className="font-bold text-sm text-gray-900 uppercase">{work.company}</Text>
                        </Box>
                        <Text className="text-sm text-gray-700 font-semibold mb-2">{work.workName || work.title}</Text>
                        <Box
                            className={`inline-block py-1 px-2 rounded text-xs font-semibold border ${getStatusColor(
                                work.status
                            )}`}
                        >
                    ...
                    <Box className="space-y-2">
                        {(work.serviceType || work.service) && (
                            <Box className="p-2 bg-gray-50 rounded-lg border border-gray-200">
                                <Text className="text-xs text-gray-600 font-semibold mb-0.5">Loại Dịch Vụ</Text>
                        ...
                        {work.equipment && (
                            <Box className="p-2 bg-gray-50 rounded-lg border border-gray-200">
                                <Text className="text-xs text-gray-600 font-semibold mb-0.5">Thiết Bị</Text>
                                <Text className="text-xs font-semibold text-gray-900">{work.equipment}</Text>
                            </Box>
                        )}
                        {work.priority && (
                            <Box className="p-2 bg-gray-50 rounded-lg border border-gray-200">
                                <Text className="text-xs text-gray-600 font-semibold mb-0.5">Ưu Tiên</Text>
                                <Text className="text-xs font-semibold text-gray-900">
                                    {work.priority === "high"
                                        ? "Cao"
                    ...
                    <Box className="space-y-2">
                        {work.customerName && (
                            <Box className="p-2 bg-gray-50 rounded-lg border border-gray-200">
                                ...
                                <Text className="text-xs font-semibold text-gray-900">{work.customerName}</Text>
                            </Box>
                        )}
                        {work.phoneNumber && (
                            <Box
                                ...
                                <Box className="flex items-center justify-between gap-2">
                                    <Box className="flex-1 min-w-0">
                                        ...
                                        <Text className="text-xs text-gray-500 mt-0.5">
                                            Sao chép để dán vào Google Maps
                                        </Text>
                                    </Box>
                                    <button
                ...
                {work.notes && (
                    <Box>
                        ...
                        <Box className="p-2 bg-gray-50 rounded-lg border border-gray-200">
                            <Text className="text-xs leading-relaxed text-gray-800 whitespace-pre-wrap">
                                {work.notes}
                            </Text>
                        </Box>
                    </Box>
