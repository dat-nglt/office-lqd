/*
 * Dữ liệu cần thiết cho trang WorkReportList:
 * - workReports: Mảng các đối tượng báo cáo với id (số), date (chuỗi), company (chuỗi), content (chuỗi), customerName (chuỗi), estimatedStartTime (chuỗi HH:MM), estimatedEndTime (chuỗi HH:MM), status (chuỗi như "completed"). Được sử dụng để hiển thị danh sách báo cáo công việc.
 * - filterDate: Chuỗi ngày (YYYY-MM-DD) để lọc báo cáo theo ngày.
 *
 * API cần thiết (đề xuất thực hiện):
 * - fetchWorkReports(employeeId, date): API để lấy danh sách báo cáo công việc từ backend dựa trên ID nhân viên và ngày. Ví dụ: GET /api/work-reports/list?employeeId=123&date=2024-01-15. Trả về mảng workReports.
 * - Cải tiến tiềm năng: Tích hợp useEffect để gọi fetchWorkReports khi component mount hoặc khi filterDate thay đổi; thêm xử lý lỗi và loading states; sử dụng Axios hoặc Fetch cho các API backend.
