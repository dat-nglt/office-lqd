                ...
                try {
                    ...
                    if (response && response.success && Array.isArray(response.data)) {
                        ...
                        const transformedWorkList = response.data.map((work) => {
                            ...
                            const requiredDateStr = requiredDateObj
                                ? requiredDateObj.toLocaleDateString("vi-VN")
                                : "Không xác định";

                            // Parse completed date safely
                            const completedDateObj = work.completed_date ? new Date(work.completed_date) : null;
                            const completedDateStr = completedDateObj
                                ? completedDateObj.toLocaleDateString("vi-VN")
                                : null;
                            ...
                            return {
                                ...
                                scheduledTime: `${work.required_time_hour || "00"}:${String(
                                    work.required_time_minute || 0
                                ).padStart(2, "0")}`,
                    ...
                    setError(
                        "Không thể tải danh sách công việc. Vui lòng thử lại sau khi hoàn thành bảo trì hệ thống."
                    );
                    toast?.show?.({
                        type: "error",
    ...
    return (
        <Page className="bg-gray-50 min-h-screen pb-20">
            ...
            <Box className="bg-gradient-to-r from-green-600 via-green-700 to-green-800 shadow-lg pb-4 relative overflow-hidden">
                ...
                <Box className="px-4 pt-10 mt-5 relative z-10">
                    {/* Title */}
                            ...
                            <Button
                                ...
                                className={`flex-1 rounded-lg text-xs ${
                                    selectedPeriod === option.value
                                        ...
                                        : "border border-gray-200 bg-transparent text-green-600"
                                }`}
